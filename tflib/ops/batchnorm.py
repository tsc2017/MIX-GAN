...
def Batchnorm(name, axes, inputs, is_training=None, stats_iter=None, update_moving_stats=True, fused=True):
    ...
    if ((axes == [0,1,2]) or (axes == [0,1])) and fused==True:
        if axes==[0,1]:
        ...
        offset = lib.param(name+'.offset', np.zeros(inputs.get_shape()[3], dtype=dtype))
        scale = lib.param(name+'.scale', np.ones(inputs.get_shape()[3], dtype=dtype))

        moving_mean = lib.param(name+'.moving_mean', np.zeros(inputs.get_shape()[3], dtype=dtype), trainable=False)
        moving_variance = lib.param(name+'.moving_variance', np.ones(inputs.get_shape()[3], dtype=dtype), trainable=False)

        def _fused_batch_norm_training():
            return tf.compat.v1.nn.fused_batch_norm(inputs, scale, offset, epsilon=1e-5)
        def _fused_batch_norm_inference():
            # Version which blends in the current item's statistics
            batch_size = tf.cast(tf.shape(inputs)[0], dtype)
        ...
        if is_training is None:
            ...
            outputs, batch_mean, batch_var = tf.cond(is_training,_fused_batch_norm_training, _fused_batch_norm_inference)
            if update_moving_stats:
                no_updates = lambda: outputs
                def _force_updates():
                    """Internal function forces updates moving_vars if is_training."""
                    float_stats_iter = tf.cast(stats_iter, dtype)
        ...
        if axes == [0,1]:
            ...
            return tf.cast(outputs,input_type)
    else:
        ...
        if 0 not in axes:
            ...
            shape[0] = 1
        offset = lib.param(name+'.offset', np.zeros(shape, dtype=dtype))
        scale = lib.param(name+'.scale', np.ones(shape, dtype=dtype))
