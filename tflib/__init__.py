...
def get_param(name, *args, **kwargs):
    """
    A wrapper for `tf.get_variable` which enables parameter sharing in models.
    
    Creates and returns theano shared variables similarly to `tf.get_variable`, 
    except if you try to create a param with the same name as a 
    previously-created one, `param(...)` will just return the old one instead of 
    making a new one.
    ...
    if name not in _params:
        ...
        if 'ema' in name:
            kwargs['trainable']=False
        param = tf.compat.v1.get_variable(name, *args, **kwargs)
        param.param = True
        _params[name] = param
    result = _params[name]
    i = 0
...
def params_with_name(name1, name2=None):
    if name2:
        return [p for n,p in _params.items() if name1 in n and name2 in p.name]
    else:
        return [p for n,p in _params.items() if name1 in n]
